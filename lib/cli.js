...
function cmdInit() {
  ...
  // Claude Code skill — .claude/commands/n8n-creator.md
  const claudeCommandsDir = path.join(CWD, '.claude', 'commands');
  const claudeCommandDest = path.join(claudeCommandsDir, 'n8n-creator.md');
  fs.mkdirSync(claudeCommandsDir, { recursive: true });
  if (!fs.existsSync(claudeCommandDest)) {
    fs.copyFileSync(skillSrc, claudeCommandDest);
    ok('Claude Code skill installed → .claude/commands/n8n-creator.md');
  } else {
    info('Claude Code skill already exists — skipped');
  }
  ...
  Next steps:
    ...
    2. Run: npx @lhi/n8n-creator audit      — verify tdd-audit works
    3. Run: npx @lhi/n8n-creator health     — check n8n connectivity
...
function cmdHealth() {
  ...
  const envFile = path.join(CWD, '.env');
  if (!fs.existsSync(envFile)) {
    error('.env not found — run: npx @lhi/n8n-creator init');
  }
...
function cmdHelp() {
  console.log(`
${CYAN}@lhi/n8n-creator${RESET} — Secure n8n workflow builder
...
${CYAN}Usage:${RESET}
  ...
  npx @lhi/n8n-creator audit
  npx @lhi/n8n-creator health

${CYAN}Claude Code skill:${RESET}
