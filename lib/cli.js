...
// Validate that an --output= path stays within CWD (prevents path traversal)
function safeWriteOutput(outputArg, content) {
  const resolved = path.resolve(CWD, outputArg);
  const rel      = path.relative(CWD, resolved);
  if (rel.startsWith('..') || path.isAbsolute(rel)) {
    error(`--output path must be within the project directory.\n  Rejected: ${outputArg}\n  Resolved: ${resolved}`);
  }
...
function cmdValidate(args) {
  ...
  for (const file of jsonFiles) {
    ...
    // Required fields — check existence AND type to prevent downstream type confusion
    if (!wf.name || typeof wf.name !== 'string')
      issues.push('Missing or invalid "name" field (must be a string)');
    if (!Array.isArray(wf.nodes))
      issues.push('Missing or invalid "nodes" field (must be an array)');
    if (!wf.connections || typeof wf.connections !== 'object' || Array.isArray(wf.connections))
      issues.push('Missing or invalid "connections" field (must be an object)');
    ...
    if (Array.isArray(wf.nodes)) {
      for (const node of wf.nodes) {
        ...
        // Hardcoded credential heuristic — expanded key name coverage
        const paramStr = JSON.stringify(node.parameters || {});
        const credPattern = /"(password|secret|secretKey|apiKey|api_key|token|accessToken|bearerToken|clientSecret|client_secret|privateKey|private_key|authToken|auth_token)"\s*:\s*"[^"]{6,}"/i;
        if (credPattern.test(paramStr)) {
...
function cmdDeploy(args) {
  ...
  for (const fp of targets) {
    let wf;
    try {
      wf = JSON.parse(fs.readFileSync(fp, 'utf8'));
    } catch (e) {
      fail(`Cannot read workflow at ${path.relative(CWD, fp)}: ${e.message}`);
      failCount++;
      continue;
    }
    ...
    try {
      ...
      if (existing) {
        ...
        if (!created.id) throw new Error('n8n API did not return a workflow id — check your API key and n8n version');
...
function cmdHealth() {
  ...
  try {
    // Use curlSync (spawnSync args array) — no shell interpolation of user-controlled values
    const healthStatus = curlSync([
      '-s', '-o', '/dev/null', '-w', '%{http_code}',
      `${apiUrl}/healthz`,
    ]).trim();
    healthStatus === '200' ? ok('Health: OK') : warn(`Health: HTTP ${healthStatus}`);

    const apiStatus = curlSync([
      '-s', '-o', '/dev/null', '-w', '%{http_code}',
      `${apiUrl}/api/v1/workflows?limit=1`,
      '-H', `X-N8N-API-KEY: ${apiKey}`,
    ]).trim();
    ...
    if (apiStatus === '200') {
      const raw = curlSync([
        '-s',
        `${apiUrl}/api/v1/workflows?active=true&limit=100`,
        '-H', `X-N8N-API-KEY: ${apiKey}`,
      ]);
      try {
        const parsed = JSON.parse(raw);
        info(`Active workflows: ${(parsed.data || []).length}`);
      } catch (_) {
        warn('Could not parse workflow count from response');
      }
    }
  } catch (e) {
    warn(`Could not reach n8n — is it running? (${e.message})`);
  }
}
...
module.exports = { run, cmdHealth, cmdValidate, cmdDeploy, cmdReport, cmdLearn, _test: { curlSync, resolveIdeTargets } };
